...
def train_agent(env, agents, random_agent, epochs=1000, alpha=0.1, gamma=0.9, epsilon=0.1, plot_interval=25):
    ...
    for epoch in tqdm.tqdm(range(epochs)):
        ...
        for i in range(0, len(state_list) - 1):
            env.state = state_list[i+1]
            env.step(agents[(player+1)%2]._act_eval(state_list[i+1]))
            agents[player].update(state_list[i], action_list[i], reward_list[i], env.state, alpha, gamma)
        pass
